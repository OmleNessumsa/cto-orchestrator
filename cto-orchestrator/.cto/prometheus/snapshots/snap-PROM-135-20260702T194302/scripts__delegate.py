...
def delegate_to_agent(prompt: str, model: str = "sonnet", timeout: int = 600, skip_permissions: bool = False, thinking_budget: int = None, agent_role: str = "rick", team_id: Optional[str] = None, task_budget: Optional[int] = None, effort: Optional[str] = None, stream: bool = True, session_id: Optional[str] = None, ticket_id: Optional[str] = None) -> str:
...
def cmd_delegate(args):
    ...
    try:
        output = delegate_to_agent(prompt, model=model, timeout=args.timeout, skip_permissions=True, thinking_budget=thinking_budget, agent_role=agent, team_id=team_id, task_budget=task_budget, effort=effort, stream=not getattr(args, 'no_stream', False), session_id=resume_session_id, ticket_id=ticket["id"])
