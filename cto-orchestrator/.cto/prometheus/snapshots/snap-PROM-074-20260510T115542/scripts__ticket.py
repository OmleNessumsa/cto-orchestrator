...
try:
    ...
    # Fallback implementations if security_utils not found
...
def load_ticket(root: Path, ticket_id: str) -> dict:
    # Sanitize ticket_id to prevent path traversal
    try:
    ...
    fp = tickets_dir(root) / f"{safe_id}.json"
    if not fp.exists():
        print(f"Error: Ticket {safe_id} not found.", file=sys.stderr)
        sys.exit(1)
    with open(fp) as f:
        return json.load(f)
...
def save_ticket(root: Path, ticket: dict):
    ...
    # Sanitize ticket ID before writing to filesystem
    try:
        safe_id = sanitize_ticket_id(ticket['id'])
    except ValueError as e:
        print(f"Error: Invalid ticket ID: {e}", file=sys.stderr)
        sys.exit(1)
    fp = td / f"{safe_id}.json"
    with open(fp, "w") as f:
